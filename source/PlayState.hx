...
class PlayState extends MusicBeatState
{
	...
	var author:Map<String, String> = new Map<String, String>();
	...
	override public function create()
	{
        ...
        author['the-stranger'] = 'Po4lr Chilly and ZackGM';
        author['the-rapper'] = 'Po4lr Chilly and A Coder Named Dan';
        author['the-battle'] = 'Po4lr Chilly';
        author['out-of-water'] = 'Po4lr Chilly';
        author['awesome-store'] = 'Po4lr Chilly';
        author['on-my-way'] = 'Po4lr Chilly';
		author['no-bullies'] = 'Hooda the Antagonist (TGT Mod)';
		author['faker'] = 'MarStarBro (Sonic.exe Mod)';

		Application.current.window.title = 'The Funkin World of Gumball - ${author.get(SONG.song.toLowerCase())} - ${SONG.song.toUpperCase()} [${['Easy', 'Normal', 'Hard', 'G-Side'][storyDifficulty]}]';
		...
		switch (curStage)
		{
			...
			case 'cumball':
				var dahouse:BGSprite = new BGSprite('gumballBG/GumballWeek_BG', -1200, -100, 0.9, 0.9);
				dahouse.active = false;
				dahouse.scale.x = 1.3;
				dahouse.scale.y = 1.3;
				add(dahouse);
			case 'cumwin':
				var daroom:BGSprite = new BGSprite('darwinBG/DarwinWeek_BG', -1200, -100, 0.9, 0.9);
				daroom.active = false;
				daroom.scale.x = 1.3;
				daroom.scale.y = 1.3;
				add(daroom);
		...
		timeTxt.setFormat(Paths.font(FlxG.save.data.translationCS == 1 ? "vcr.ttf" : "Gumball.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font(FlxG.save.data.translationCS == 1 ? "vcr.ttf" : "Gumball.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font(FlxG.save.data.translationCS == 1 ? "vcr.ttf" : "Gumball.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		var overlay:FlxSprite = new FlxSprite().loadGraphic(Paths.image('overlay'));
	...
	override public function update(elapsed:Float)
	{
		...
		var scoreTranslation:Array<Dynamic> = [
			['Score', 'Combo Breaks', 'Rating'],
			['Счёт', 'Проваленные Комбо', 'Рейтинг'],
			['Cuenta', 'La Srtaes', 'Clasificación'],
			['Konto', 'Fehlermeldung', 'Rang'],
			['Wynik', 'Nietrafione', 'Ranking'],
			['Pontuação', 'Falhas', 'Ranking'],
			['Pontuação', 'Falhas', 'Ranking'],
			['Score', 'Erreurs', 'Classement']
		];

		if(ratingName == '?') {
			scoreTxt.text = '${scoreTranslation[FlxG.save.data.translationCS][0]}: ' + songScore + ' | ${scoreTranslation[FlxG.save.data.translationCS][1]}: ' + songMisses + ' | ${scoreTranslation[FlxG.save.data.translationCS][2]}: ' + ratingName;
		} else {
			scoreTxt.text = '${scoreTranslation[FlxG.save.data.translationCS][0]}: ' + songScore + ' | ${scoreTranslation[FlxG.save.data.translationCS][1]}: ' + songMisses + ' | ${scoreTranslation[FlxG.save.data.translationCS][2]}: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(iconP1.width, 150, 0.09 / (ClientPrefs.framerate / Conductor.bpm))));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(iconP2.width, 150, 0.09 / (ClientPrefs.framerate / Conductor.bpm))));
	...
	function makeSomeone(who:Int)
	{
		var poopy:FlxSprite;
		if(FlxG.random.bool(50))
		{
			canSomeoneWalk = false;
			poopy = new FlxSprite(-2945, FlxG.height-185).loadGraphic(Paths.image('gumballBG/char' + who, 'shared'));
			poopy.antialiasing = ClientPrefs.globalAntialiasing;
			poopy.flipX = true;
			FlxTween.tween(poopy, {x: 2945}, 25-(who == 6 ? 7 : 0), {ease: FlxEase.expoOut, onComplete: function(twn:FlxTween)
				{
					poopy.kill();
					canSomeoneWalk = true;
				}
			});
			FlxTween.tween(poopy, {y: FlxG.height-145}, 3, {ease: FlxEase.expoInOut, type: PINGPONG});
		}
		else
		{
			canSomeoneWalk = false;
			poopy = new FlxSprite(2945, FlxG.height-185).loadGraphic(Paths.image('gumballBG/char' + who, 'shared'));
			poopy.antialiasing = ClientPrefs.globalAntialiasing;
			FlxTween.tween(poopy, {x: -3600}, 25-(who == 6 ? 7 : 0), {ease: FlxEase.expoOut, onComplete: function(twn:FlxTween)
				{
					poopy.kill();
					canSomeoneWalk = true;
				}
		...
		poopy.scrollFactor.set(1.2, 1.2);
		poopy.scale.set(0.5, 0.5);
		add(poopy);
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
		    ...
		    case 'More Next':
                var more:FlxSprite = new FlxSprite().loadGraphic(Paths.image('morenext'));
                more.cameras = [camHUD];
                add(more);
                FlxTween.tween(more, {y: -45}, 1.5, {ease: FlxEase.backOut, type: BACKWARD, onComplete: function(twn:FlxTween) {
                    FlxTween.tween(more, {y: -100, alpha: 0}, 1, {ease: FlxEase.backIn, onComplete: function(twn:FlxTween)
                    {
                        more.kill();
                    }});
                }});
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'cumball':
				...
				{
					makeSomeone(FlxG.random.int(0, 10));
				}	
			case "philly":
				if (!trainMoving)
					trainCooldown += 1;
